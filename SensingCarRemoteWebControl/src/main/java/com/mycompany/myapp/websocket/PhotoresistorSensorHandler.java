...
public class PhotoresistorSensorHandler extends TextWebSocketHandler implements ApplicationListener {
	...
	public void init() {
		...
		coapClient.setURI("coap://192.168.3.42/photoresistor");
		coapObserveRelation = coapClient.observe(new CoapHandler() {			
			@Override
			public void onLoad(CoapResponse response) {
				String json = response.getResponseText();
				JSONObject jsonObject = new JSONObject(json);
				double photoresistorT = Double.parseDouble(jsonObject.getString("photoresistor"));
				double photoresistor = ((int)(photoresistorT*10))/10.0;
